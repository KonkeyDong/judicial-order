...
renderer_draw_magic_icon :: proc(scale: f32, family: defs.Magic_Family, position: rl.Vector2) {
	sprite := magic_icons_get(family)
	renderer_draw(scale, sprite, position, 255, false)
}

renderer_draw_command_icon :: proc(scale: f32, icon: defs.Command_Icon, position: rl.Vector2) {
	sprite := command_icons_get(icon)
	renderer_draw(scale, sprite, position, 255, false)
}
...
renderer_draw_item_icon :: proc(
	...
	sprite := item_icons_get_selected(name, is_selected)
	renderer_draw(scale, sprite, position, 255, false)
}
