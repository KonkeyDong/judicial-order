...
State_Procs :: struct {
	enter:        proc(game: ^Game),
	exit:         proc(game: ^Game),
	handle_input: proc(game: ^Game),
	update:       proc(game: ^Game),
	draw:         proc(game: ^Game, scale: f32),
}

state_noop :: proc(_: ^Game) {}
state_noop_draw :: proc(_: ^Game, _: f32) {}
...
state_draw_map :: proc(game: ^Game, scale: f32, draw_range, draw_highlight: bool) {
...
state_change :: proc(game: ^Game, kind: defs.State_Kind) {
	if game == nil {
...
state_show_message_notice :: proc(game: ^Game, message: string, return_state: defs.State_Kind) {
	if game == nil {
		log.errorf("state_show_message_notice: game is nil.")
		return
	}

	message_notice_set(&game.message_notice, message, return_state)
	state_change(game, .MessageNotice)
}

state_enter :: proc(game: ^Game) {
	if game == nil {
...
state_handle_input :: proc(game: ^Game) {
	if game == nil {
