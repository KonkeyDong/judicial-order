...
state_item_handle_slot_keys :: proc(
	...
	filter: Item_Slot_Filter,
) {
	if input_key_pressed(.UP) {
		item_ui_set_selected(&game.item_ui, .Up, caster, filter)
	}

	if input_key_pressed(.LEFT) {
		item_ui_set_selected(&game.item_ui, .Left, caster, filter)
	}

	if input_key_pressed(.RIGHT) {
		item_ui_set_selected(&game.item_ui, .Right, caster, filter)
	}

	if input_key_pressed(.DOWN) {
		item_ui_set_selected(&game.item_ui, .Down, caster, filter)
	}
}

state_draw_item_radial :: proc(game: ^Game, scale: f32, owner: ^unit_pkg.Unit) {
	if owner == nil {
		return
	}
	...
	for direction in directions {
		...
		renderer_draw_item_icon(
			scale,
			slot.name,
			radial_icon_position(game.item_ui.center, direction),
			selected,
		)
	}
}

battle_item_menu_enter :: proc(game: ^Game) {
...
battle_item_menu_exit :: proc(_: ^Game) {}

battle_item_menu_handle_input :: proc(game: ^Game) {
	if input_key_pressed(.UP) {
		game.state_scratch.selected_command = .Use
	}
...
battle_item_menu_confirm :: proc(game: ^Game) {
	current := game_current_unit(game)
	if current == nil {
		return
	}
...
battle_item_menu_update :: proc(game: ^Game) {
	timers.oscillator_tick(&game.grid.range_tint)
	timers.flip_flop_tick(&game.flip_flop)
}

battle_item_menu_draw :: proc(game: ^Game, scale: f32) {
...
use_which_item_enter :: proc(game: ^Game) {
	current := game_current_unit(game)
	item_ui_reset(&game.item_ui)
	item_ui_reset_layout_center(game)
	item_ui_select_first(&game.item_ui, current, item_ui_usable_filter)
	if !item_ui_has_valid_selection(&game.item_ui, current, item_ui_usable_filter) {
		state_show_message_notice(game, defs.MESSAGE_NOTICE.no_item, .BattleItemMenu)
		return
	}

	if current != nil {
		grid_calculate_item_use_range(&game.grid, current, item_ui_selected_data(&game.item_ui))
	}
}

use_which_item_exit :: proc(_: ^Game) {}

use_which_item_handle_input :: proc(game: ^Game) {
	current := game_current_unit(game)
	state_item_handle_slot_keys(game, current, item_ui_usable_filter)
	if item_ui_has_valid_selection(&game.item_ui, current, item_ui_usable_filter) &&
	   current != nil {
		grid_calculate_item_use_range(&game.grid, current, item_ui_selected_data(&game.item_ui))
	}
...
use_which_item_update :: proc(game: ^Game) {
	timers.oscillator_tick(&game.grid.range_tint)
	timers.flip_flop_tick(&game.flip_flop)
	sprites.item_icons_tick()
}

use_which_item_draw :: proc(game: ^Game, scale: f32) {
	state_draw_map(game, scale, true, false)
	state_draw_item_radial(game, scale, game_current_unit(game))
	renderer_draw_item_info_box(
		scale,
		item_ui_selected_data(&game.item_ui),
		false,
		game.item_ui.info_box,
	)
}

use_item_on_whom_enter :: proc(game: ^Game) {
	current := game_current_unit(game)
	if current == nil {
		return
	}
...
use_item_on_whom_push_target :: proc(game: ^Game, target: ^unit_pkg.Unit) {
...
use_item_on_whom_enter_heal_all :: proc(game: ^Game, current: ^unit_pkg.Unit) {
...
use_item_on_whom_enter_consumable :: proc(
	...
	grid_calculate_item_use_range(&game.grid, current, data)
	...
	if current.on_map && grid_in_range(&game.grid, current.grid_x, current.grid_y) {
	...
	game_set_highlight_target(game, game.state_scratch.targets[0])
}
...
use_item_on_whom_enter_spell :: proc(
	...
	grid_calculate_magic_attack_range(&game.grid, current, magic)
	units := grid_units_in_range(&game.grid)
	defer delete(units)
	for unit in units {
		if unit == nil {
	...
	if !magic.offensive &&
	   current.on_map &&
	   grid_in_range(&game.grid, current.grid_x, current.grid_y) {
		use_item_on_whom_push_target(game, current)
	}
...
use_item_on_whom_exit :: proc(_: ^Game) {}

use_item_on_whom_handle_input :: proc(game: ^Game) {
	if input_try_cycle_index(&game.state_scratch.list_index, game.state_scratch.target_count) {
...
use_item_on_whom_confirm_consumable_targets :: proc(game: ^Game) {
	current := game_current_unit(game)
	item_context_init(
		&game.item_context,
		current,
...
use_item_on_whom_confirm_spell :: proc(game: ^Game) {
	current := game_current_unit(game)
	...
	grid_calculate_spell_effect_range(&game.grid, selected, magic)
	...
	magic_context_cast(&game.magic_context, data.spell_name, true)
...
use_item_on_whom_update :: proc(game: ^Game) {
	timers.oscillator_tick(&game.grid.range_tint)
	timers.flip_flop_tick(&game.flip_flop)
	game_update_highlight(game, rl.GetFrameTime())
}

use_item_on_whom_draw :: proc(game: ^Game, scale: f32) {
	state_draw_map(game, scale, true, true)
}

drop_item_enter :: proc(game: ^Game) {
	current := game_current_unit(game)
	if current != nil {
		grid_calculate_unit_movement_range(&game.grid, current)
	}
	...
	item_ui_reset_layout_center(game)
	item_ui_set_selected(&game.item_ui, .Up, current, item_ui_giveable_filter)
}

drop_item_exit :: proc(_: ^Game) {}

drop_item_handle_input :: proc(game: ^Game) {
	current := game_current_unit(game)
	state_item_handle_slot_keys(game, current, item_ui_giveable_filter)
	if input_confirm_press() {
...
drop_item_update :: proc(game: ^Game) {
	timers.oscillator_tick(&game.grid.range_tint)
	timers.flip_flop_tick(&game.flip_flop)
	sprites.item_icons_tick()
}

drop_item_draw :: proc(game: ^Game, scale: f32) {
	state_draw_map(game, scale, true, false)
	state_draw_item_radial(game, scale, game_current_unit(game))
}
...
equip_item_enter :: proc(game: ^Game) {
	current := game_current_unit(game)
...
equip_item_exit :: proc(_: ^Game) {}

equip_item_handle_input :: proc(game: ^Game) {
	current := game_current_unit(game)
	if input_key_pressed(.UP) && equip_item_slot_equippable(current, 0) {
		game.state_scratch.list_index = 0
		game.state_scratch.equip_unarmed = false
	}

	if input_key_pressed(.LEFT) && equip_item_slot_equippable(current, 1) {
		game.state_scratch.list_index = 1
		game.state_scratch.equip_unarmed = false
	}

	if input_key_pressed(.RIGHT) && equip_item_slot_equippable(current, 2) {
...
equip_item_update :: proc(game: ^Game) {
	timers.flip_flop_tick(&game.flip_flop)
	sprites.item_icons_tick()
}

equip_item_draw :: proc(game: ^Game, scale: f32) {
	state_draw_map(game, scale, false, false)
	state_draw_item_radial(game, scale, game_current_unit(game))
}

give_which_item_enter :: proc(game: ^Game) {
	current := game_current_unit(game)
	if current != nil {
		grid_calculate_give_range(&game.grid, current)
		units := grid_units_in_range(&game.grid)
		defer delete(units)
	...
	item_ui_reset(&game.item_ui)
	item_ui_reset_layout_center(game)
	item_ui_select_first(&game.item_ui, current, item_ui_giveable_filter)
}

give_which_item_exit :: proc(_: ^Game) {}

give_which_item_handle_input :: proc(game: ^Game) {
	current := game_current_unit(game)
	state_item_handle_slot_keys(game, current, item_ui_giveable_filter)
	if input_confirm_press() {
		if !item_ui_has_valid_selection(&game.item_ui, current, item_ui_giveable_filter) {
			return
		}
...
give_which_item_update :: proc(game: ^Game) {
	timers.oscillator_tick(&game.grid.range_tint)
	timers.flip_flop_tick(&game.flip_flop)
	sprites.item_icons_tick()
}

give_which_item_draw :: proc(game: ^Game, scale: f32) {
	state_draw_map(game, scale, true, false)
	state_draw_item_radial(game, scale, game_current_unit(game))
}

give_item_to_whom_enter :: proc(game: ^Game) {
	current := game_current_unit(game)
	if current != nil {
		grid_calculate_give_range(&game.grid, current)
		units := grid_units_in_range(&game.grid)
	...
	game.state_scratch.list_index = 0
	game_initialize_highlight(game)
	game_set_highlight_target(game, game.friendly_units_in_range[0])
}

give_item_to_whom_exit :: proc(_: ^Game) {}

give_item_to_whom_handle_input :: proc(game: ^Game) {
	if input_try_cycle_index(&game.state_scratch.list_index, len(game.friendly_units_in_range)) {
...
give_item_to_whom_update :: proc(game: ^Game) {
	timers.oscillator_tick(&game.grid.range_tint)
	timers.flip_flop_tick(&game.flip_flop)
	sprites.item_icons_tick()
	game_update_highlight(game, rl.GetFrameTime())
...
give_item_to_whom_draw :: proc(game: ^Game, scale: f32) {
	state_draw_map(game, scale, true, true)
	if len(game.friendly_units_in_range) == 0 {
		return
	}
	...
	recipient := game.friendly_units_in_range[game.state_scratch.list_index]
	renderer_draw_unit_info_box(scale, recipient, defs.GIVE.positions.recipient_info_box)
}

trade_which_item_enter :: proc(game: ^Game) {
	current := game_current_unit(game)
	...
	item_ui_reset(&game.item_ui)
	item_ui_set_layout_center(&game.item_ui, defs.GIVE.positions.trade_inventory_center)
	item_ui_select_first(&game.item_ui, recipient, item_ui_giveable_filter)
	if recipient.on_map {
		game_initialize_highlight(game)
		game_set_highlight_target(game, recipient)
	}
}

trade_which_item_exit :: proc(_: ^Game) {}

trade_which_item_handle_input :: proc(game: ^Game) {
	recipient := game.give.recipient
	state_item_handle_slot_keys(game, recipient, item_ui_giveable_filter)
	if input_confirm_press() {
		if !item_ui_has_valid_selection(&game.item_ui, recipient, item_ui_giveable_filter) {
			return
		}
...
trade_which_item_update :: proc(game: ^Game) {
	timers.oscillator_tick(&game.grid.range_tint)
	timers.flip_flop_tick(&game.flip_flop)
	sprites.item_icons_tick()
	game_update_highlight(game, rl.GetFrameTime())
...
trade_which_item_draw :: proc(game: ^Game, scale: f32) {
	state_draw_map(game, scale, true, true)
	state_draw_item_radial(game, scale, game.give.recipient)
}

prompt_yes_no_enter :: proc(game: ^Game) {
	game.state_scratch.yes_selected = true
}

prompt_yes_no_exit :: proc(_: ^Game) {}

prompt_yes_no_handle_input :: proc(game: ^Game) {
	if input_key_pressed(.LEFT) {
		game.state_scratch.yes_selected = true
	}
...
prompt_yes_no_on_yes :: proc(game: ^Game) {
	current := game_current_unit(game)
...
prompt_yes_no_on_no :: proc(game: ^Game) {
	next := game.prompt.return_state_on_no
	prompt_reset(&game.prompt)
	state_change(game, next)
}

prompt_yes_no_update :: proc(game: ^Game) {
	timers.flip_flop_tick(&game.flip_flop)
}

prompt_yes_no_draw :: proc(game: ^Game, scale: f32) {
	state_draw_map(game, scale, false, false)
	label := "Yes" if game.state_scratch.yes_selected else "No"
	renderer_draw_battle_menu_message(scale, label, radial_center(game))
}
